                ...
                for lesson in lessons:
                    ...
                    if 0 <= delta <= mins + 1 and key not in _reminder_sent:
                        ...
                        if text:
                            ...
                            try:
                                await bot.send_message(tid, f'⏰ Через {mins} мин начинается пара!\n\n📚 {html.escape(first_line)}\n🕐 {lesson.time}', parse_mode='HTML')
                        ...
                        if row.final_score is not None:
                            ...
                            if old_score is not None and old_score != row.final_score:
                                try:
                                    ...
                                    logger.warning(f'Ошибка отправки уведомления об оценке {tid}: {e}')
                except Exception as e:
                    logger.warning(f'Ошибка обновления БРС для {tid}: {e}')
        except Exception as e:
            logger.error(f'Grade check loop error: {e}')
