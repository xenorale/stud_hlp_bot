...
def format_brs_subject(row: BrsRow) -> str:
    ...
    weighted_display = f"{row.weighted_score} {penalty_info['grade']}" if row.weighted_score is not None else '—'
    final_score_str = f'{row.final_score} ({row.final_text})' if row.final_score is not None else row.final_text or '—'
    return f"📚 **{row.subject}**\n   Семестр {row.semester} | {row.control}\n   Преподаватель: {row.teacher_short}\n   Аттестации: {att_str}\n   Посещаемость: {att_pct_str}\n   Взвешенный балл: {weighted_display}\n   Экзамен: {(row.exam_score if row.exam_score else '—')}\n   Итог: {final_score_str}"

def get_brs_data(username: str, password: str, student_id: str) -> Optional[List[BrsRow]]:
    try:
