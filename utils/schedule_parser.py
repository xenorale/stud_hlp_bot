...
XLSX_URL = f'https://docs.google.com/spreadsheets/d/{SHEET_ID}/export?format=xlsx&gid={SHEET_GID}'
_MERGE_CACHE: dict = {'merges': None, 'ts': 0}
_MERGE_TTL = 86400
DAYS_ORDER = ['Понедельник', 'Вторник', 'Среда', 'Четверг', 'Пятница', 'Суббота']
_TIME_RE = re.compile('^\\d{1,2}:\\d{2}')
_COURSE_RE = re.compile('^(\\d+)\\s+курс$')
_GROUP_RE = re.compile('^(\\d+)\\s+группа$')
_TEACHER_RE = re.compile('(доц\\.|ст\\.преп\\.|преп\\.|асс\\.|проф\\.)\\s+(.+?)(?:\\s+(\\d+\\S*))?\\s*$')
...
def _same_merge(row: int, col_a: int, col_b: int, merges: list[tuple[int, int, int, int]]) -> bool:
...
def _find_subsection_start(rows: list[list[str]], col: int, course_num: int) -> int:
    row0 = rows[0]
    starts = [i for i, cell in enumerate(row0) if _COURSE_RE.match(cell.strip()) and int(_COURSE_RE.match(cell.strip()).group(1)) == course_num and (i <= col)]
...
def parse_group_schedule(group_num: int, course_num: int) -> list[Lesson]:
    ...
    if col is None:
        raise ValueError(f'Группа {group_num} не найдена на {course_num} курсе. Проверь номер курса и группы в профиле.')
    row1 = rows[1]
    ...
    while i < len(rows):
        ...
        # Если в знаменателе пусто, проверяем: не является ли ячейка числителя
        # объединенной (merged) на две строки. Если да — это общая пара на обе недели.
        if not sub1_den and sub1_num and _same_merge(i-1, col, col, merges):
            # Проверяем, захватывает ли этот мерж следующую строку
            for r_min, r_max, c_min, c_max in merges:
                if r_min <= (i-1) < r_max and c_min <= col <= c_max:
                    sub1_den = sub1_num
                    break
        if not sub2_den and sub2_num and _same_merge(i-1, col2, col2, merges):
            for r_min, r_max, c_min, c_max in merges:
                if r_min <= (i-1) < r_max and c_min <= col2 <= c_max:
                    sub2_den = sub2_num
                    break

        if sub1_num or sub1_den or sub2_num or sub2_den:
            lessons.append(Lesson(day=current_day, time=time_cell, sub1_num=sub1_num, sub1_den=sub1_den, sub2_num=sub2_num, sub2_den=sub2_den))
        i += 1
    return lessons

def has_subgroups(lessons: list[Lesson]) -> bool:
    return any((lesson.sub1_num != lesson.sub2_num or lesson.sub1_den != lesson.sub2_den for lesson in lessons))
_ID_RE = re.compile('\\s*\\(id=\\d+\\)\\s*')
_DO_RE = re.compile('\\(ДО\\s*\\)', re.IGNORECASE)
...
def _format_subject(subject: str) -> str:
    ...
    if m:
        subj_name = re.sub('\\s+', ' ', clean[:m.start()]).strip()
